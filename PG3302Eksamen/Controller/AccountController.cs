...
public class AccountController {
	...
	private string GenerateBankAccountNumber(AccountRepository accRep) {
		...
		foreach (var accNr in accRep.GetAllAccountNumbers().Where(accNr => accNr.Contains(accountNumberGenerated))) {
	...
	private void AccountTypeChooser(int personIdentifier, PersonRepository personRep, string accountNumberGenerated) {
		while (true) {
			...
			switch (savingsOrCurrentAcc) {
				case "1": {
					...
					if (string.IsNullOrEmpty(newAccountName)) {
						Ui.InvalidInputMessage(null);
continue;					}
					SendAccountToDatabase(new SavingsAccountFactory(), personIdentifier, newAccountName, personRep, accountNumberGenerated);
					break;
				}
				case "2": {
					...
					if (string.IsNullOrEmpty(newAccountName)) {
						Ui.InvalidInputMessage(null);
continue;					}
					SendAccountToDatabase(new CurrentAccountFactory(), personIdentifier, newAccountName, personRep, accountNumberGenerated);
					break;
				}
			}
